...
- uses: sidx1024/report-nyc-coverage-github-action@v1.0.0
  with:
    ...
    # Specifies if you rather have the output as a step summary than a comment. Can be
    # one of "none", "replace", "new".
    # Default: none
    comment_mode: ""

    # An alternative GitHub token, other than the default provided by GitHub Actions
    # runner. Optional. Only required when comment_mode is set to "new" or "replace".
...
| **`comment_mode`**                           | Specifies if you rather have the output as a step summary than a comment. Can be one of "none", "replace", "new".                                              |                  `none`                   |  **false**   |
| **`github_token`**                           | An alternative GitHub token, other than the default provided by GitHub Actions runner. Optional. Only required when comment_mode is set to "new" or "replace". |           `${{ github.token }}`           |  **false**   |
